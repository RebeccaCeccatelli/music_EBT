#!/bin/bash
### Baseline Transformer Music - Inference Script
### Generate MIDI music using a pretrained baseline transformer
...
#SBATCH --output=logs/slurm/mus/baseline-mus-inference_%A-%a.log

### ADDITIONAL RUN INFO ###
#SBATCH --array=0

### LOG INFO ###
#SBATCH --job-name=baseline-mus-inference
# SLURM logs sent to wandb (see --wandb_project below)
export RUN_NAME="baseline-mus-inference"
...
--only_test \
--only_test_model_ckpt "logs/checkpoints/baseline-symb-xxs-prod0.0008_2026-04-22_18-17-39_/last.ckpt" \
