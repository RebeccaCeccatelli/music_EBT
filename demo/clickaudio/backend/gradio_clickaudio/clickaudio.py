...
class WaveformOptions:
    """
    A dataclass for specifying options for the waveform display in the ClickAudio component. An instance of this class can be passed into the `waveform_options` parameter of `gr.Audio`.
    Parameters:
...
class ClickAudio(
    ...
    EVENTS = [
        ...
        EventListener(
            "seek",
            doc="This listener is triggered when the user clicks on the waveform to seek to a "
            "new playback position. Add `evt: gr.EventData` to the listener function and read "
            "`evt.time` for the clicked time, in seconds.",
        ),
    ]
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: A path, URL, or [sample_rate, numpy array] tuple (sample rate in Hz, audio data as a float or int numpy array) for the default value that ClickAudio component is going to take. If a function is provided, the function will be called each time the app loads to set the initial value of this component.
        ...
        if self.streaming and "microphone" not in self.sources:
            raise ValueError(
                "ClickAudio streaming only available if sources includes 'microphone'."
            )
    ...
    def postprocess(
        ...
        if isinstance(value, bytes):
            ...
            raise ValueError(f"Cannot process {value} as ClickAudio")
    ...
    async def covert_to_adts(data: bytes) -> tuple[bytes, float]:
        return await anyio.to_thread.run_sync(ClickAudio._convert_to_adts, data)
    ...
    def check_streamable(self):
        if (
            ...
            raise ValueError(
                "ClickAudio streaming only available if source includes 'microphone'."
            )
